...
#
# Tags look like `v<Alertmanager version>-<release>`, which is what this
# repository has been releasing all along (`v0.34.0-0`, `v0.33.0-1`, …):
#
# - if defaults/main.yml points at an Alertmanager version that has never been
#   released, the release counter restarts at 0 (`v0.34.0-0`)
# - otherwise the counter is incremented (`v0.34.0-1`), but only if something
...
# Anchored on `prometheus_alertmanager_version:`, which is the leaf variable
# that Renovate's annotation sits above and edits. The derived
# `prometheus_alertmanager_container_image_tag` must not be mistaken for it.
version="$(sed -nE 's|^prometheus_alertmanager_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Alertmanager version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.34.0`),
